...
class EmailService {
    static transporter = nodemailer_1.default.createTransport({
        ...
        secure: config_1.default.smtp.port === 465, // True for 465, false for others
        auth: {
    ...
    static async sendEmail(to, subject, html) {
        try {
            const info = await this.transporter.sendMail({
                from: config_1.default.smtp.from,
                to,
                subject,
                html,
            });
            logger_1.default.info(`📧 Email sent successfully to ${to}. MessageId: ${info.messageId}`);
            return true;
        }
        catch (error) {
